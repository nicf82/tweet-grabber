...
class TweetProcessingServiceSpec  extends AnyWordSpecLike with TestActorSystem with Fixtures with MockitoSugar with BeforeAndAfterEach {
  ...
  "Running the standardIncomingTweetFlow" should {
    ...
    "Store the tweet as a stub, request entries surrounding the tweet, but no more if the entries returned did not have track/names mentioned in the tweet" in {
      ...
      val done = Source.single(tweet2)
        ...
        .via(service.tweetEventMatchingFlow)
      ...
      verify(entryRepo, times(1)).getEntriesStream(meq(stubTweet.time), meq(stubTweet.time.minusHours(24)))(any())
      verify(newTweetRepo, times(0)).store(any(), any())
      verify(entryRepo, times(0)).attachTweet(any(), any())
      verify(entryRepo, times(0)).attachTweet(any(), any())
    }

    "Store the tweet as a stub, request entries surrounding the tweet, and when entries returned had track/names mentioned in the tweet, store the tweet in full and attach it to both matching entries" in {
